...
export class S3FilesService extends FilesService {
    ...
    private constructor(accessKey: string, secretAccessKey: string, bucketName: string, host?: string) {
        super();
    ...
    static getInstance(...args: string[]): FilesService {
        if (!FilesService.instance) {
            FilesService.instance = new S3FilesService(args[0], args[1], args[2], args[3]);
        }
    ...
    async uploadFiles(): Promise<void> {
        try {
            ...
            const uploadFile = async (fileName: string, filePath: string): Promise<void> => {
                ...
                console.log(`Uploading file ${fileName} to S3 bucket ${this.bucket} with delay ${randomDelay} ms`);
    ...
    async isBucketCreated(bucketName: string): Promise<boolean> {
        try {
            ...
            const bucketFound = !!data.Buckets?.find(b => { return b.Name === bucketName });
            return bucketFound;
        } catch (err) {
